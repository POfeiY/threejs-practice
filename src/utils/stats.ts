const Stats = function () {
  ...
  let beginTime = (performance || Date).now(); let prevTime = beginTime; let frames = 0
...
Stats.Panel = function (name: string, fg: string, bg: string) {
  let min = Infinity; let max = 0; const round = Math.round
  const PR = round(window.devicePixelRatio || 1)

  const WIDTH = 80 * PR; const HEIGHT = 48 * PR
  const TEXT_X = 3 * PR; const TEXT_Y = 2 * PR
  const GRAPH_X = 3 * PR; const GRAPH_Y = 15 * PR
  const GRAPH_WIDTH = 74 * PR; const GRAPH_HEIGHT = 30 * PR
